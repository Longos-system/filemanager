...
import Long.DragDrop 1.0 as DragDrop
...
GridView {
    ...
    cellHeight: {
        var iconHeight = iconSize + (LongUI.Units.fontMetrics.height * 2) + LongUI.Units.largeSpacing * 2
        if (isDesktopView) {
    ...
    cellWidth: {
        var iconWidth = iconSize + LongUI.Units.largeSpacing * 4
    ...
    function rectangleSelect(x, y, width, height) {
        ...
        var cWidth = control.cellWidth - (2 * LongUI.Units.smallSpacing)
        var cHeight = control.cellHeight - (2 * LongUI.Units.smallSpacing)
        ...
        for (var s = 0; s < stripes; s++) {
            for (var i = 0; i < perStripe; i++) {
                ...
                // expensive work.
                if (control.rubberBand.intersects(Qt.rect(itemX + LongUI.Units.smallSpacing, itemY + LongUI.Units.smallSpacing,
                    cWidth, cHeight))) {
        ...
        TextField {
            ...
            onTargetItemChanged: {
                if (targetItem != null) {
                    ...
                    width = targetItem.width - LongUI.Units.smallSpacing
                    height = targetItem.labelArea.paintedHeight + LongUI.Units.largeSpacing * 2
                    x = targetItem.x + Math.abs(Math.min(control.contentX, control.originX))
                    y = pos.y - LongUI.Units.smallSpacing
