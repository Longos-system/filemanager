 ...
 *   Copyright (C) 2023 chang2005 <389574063@qq.com>               *
 *                                                                         *
...
{
    QSettings settings("cuteos", qApp->applicationName());
...
void FolderModel::paste()
{
    ...
    if (enable) {
        ...
        if (mimeData->hasFormat("application/x-cute-cutselection")) {
...
void FolderModel::cut()
{
    ...
    mimeData->setData("application/x-cute-cutselection", QByteArray("1"));
...
void FolderModel::setWallpaperSelected()
{
    ...
    QDBusInterface iface("com.cute.Settings", "/Theme",
                         "com.cute.Theme",
...
{
    QProcess::startDetached("cute-settings", QStringList() << "-m" << "background");
}
...
{
    if (!url.isEmpty()) {
        QProcess::startDetached("cute-filemanager", QStringList() << url);
        return;
    }
    ...
    for (const QModelIndex &index : m_selectionModel->selectedIndexes()) {
        KFileItem item = itemForIndex(index);
        if (item.isDir()) {
            QProcess::startDetached("cute-filemanager", QStringList() << item.url().toLocalFile());
        }
    }
}
